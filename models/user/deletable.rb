module User::Deletable
  ...
  def delete_likes_on_posts
    group.posts.where('likes.owner_id' => id.to_s).each do |post|
      post.likes.where(owner_id: owner_id).destroy_all
    end
  end
  
  def delete_likes_on_comments
    group.posts.where('comments.likes.owner_id' => id.to_s).each do |post|
      post.comments.each { |c| c.likes.where(owner_id: id.to_s).destroy_all }
    end
  end
  ...
  def delete_posts
    group.posts.where(owner_id: id.to_s).destroy_all
  end

  def delete_comments
    group.posts.where('comments.owner_id' => id.to_s).each do |post|
      post.comments.where(owner_id: id.to_s).destroy_all
    end
  end
